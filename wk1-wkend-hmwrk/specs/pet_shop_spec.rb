...
class TestPetShop < Minitest::Test
  ...
  #      p "curious check cash total 1 (before): #{@pet_shop[:admin][:total_cash]}"
     add_or_remove_cash(@pet_shop,10)
     cash = total_cash(@pet_shop)
    assert_equal(1010, cash)
#        p "curious 2 (after addition): #{@pet_shop[:admin][:total_cash]}"
 end

def test_add_or_remove_cash__remove
#      p "curious 3 (before remove): #{@pet_shop[:admin][:total_cash]}"
    add_or_remove_cash(@pet_shop,-10)
    cash = total_cash(@pet_shop)
    assert_equal(990, cash)
  #  p "curious 4 (after remove): #{@pet_shop[:admin][:total_cash]}"
#    p "weird that these aren't running in the same order each time"
end
